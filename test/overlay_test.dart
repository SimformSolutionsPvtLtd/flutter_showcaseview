...
Color? findOverlayColor(WidgetTester tester, Color color) {
  ...
  for (final box in coloredBox) {
    ...
    if (boxColor != null) {
      ...
      final colorMatches = (boxColor.red - color.red).abs() < 0.01 &&
          (boxColor.green - color.green).abs() < 0.01 &&
          (boxColor.blue - color.blue).abs() < 0.01 &&
          (boxColor.opacity - color.opacity).abs() < 0.01;
  ...
  // If no exact match, try to find closest color and print for debugging
  if (coloredBox.isNotEmpty && coloredBox.first?.color != null) {
    // Return the first color we find for checking in the test
...
void main() {
  group('Overlay Tests', () {
    ...
    testWidgets('Overlay renders with default properties',
    ...
    testWidgets('Overlay with custom color and opacity',
        (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
      ...
      final color = findOverlayColor(tester, Colors.purple.withOpacity(0.5));
      expect(color, isNotNull);
      expect(color, Colors.purple.withOpacity(0.5));
    });

    testWidgets('Overlay with blur value', (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
    ...
    testWidgets('Overlay with barrier click callback',
        (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
      ...
      // Verify overlay is rendered
      expect(find.byType(Overlay), findsWidgets);
      await tester.tapAt(const Offset(10, 10.0)); // Tap on the barrier
    ...
    testWidgets('Overlay with disabled barrier interaction',
        (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
    ...
    testWidgets('Overlay with multiple showcases', (WidgetTester tester) async {
    ...
    testWidgets('Overlay with multi showcases', (WidgetTester tester) async {
    ...
    testWidgets('Overlay with different opacity values',
        (WidgetTester tester) async {
      final GlobalKey key1 = GlobalKey();
      final GlobalKey key2 = GlobalKey();
      ...
      expect(find.byType(Overlay), findsWidgets);
      var color = findOverlayColor(tester, Colors.blue.withOpacity(0.9));
      expect(color, isNotNull);

      // Allow a small tolerance when comparing colors
      expect(
        (color!.opacity - 0.9).abs() < 0.01,
        isTrue,
        reason: 'Opacity should be close to 0.9',
      );
      expect(color.red, closeTo(Colors.blue.red, 0.01));
      expect(color.green, closeTo(Colors.blue.green, 0.01));
      expect(color.blue, closeTo(Colors.blue.blue, 0.01));
      ...
      color = findOverlayColor(tester, Colors.blue.withOpacity(0.1));
      expect(color, isNotNull);

      // Allow a small tolerance when comparing colors
      expect(
        (color!.opacity - 0.1).abs() < 0.05,
        isTrue,
      ...
      color = findOverlayColor(tester, Colors.green.withOpacity(0.3));
      expect(color, isNotNull);

      // Allow a small tolerance when comparing colors
      expect(
        (color!.opacity - 0.3).abs() < 0.05,
        isTrue,
        reason: 'Opacity should be close to 0.3',
      );
      expect(color.red, closeTo(Colors.green.red, 0.01));
      expect(color.green, closeTo(Colors.green.green, 0.01));
      expect(color.blue, closeTo(Colors.green.blue, 0.01));
    });

    testWidgets('Overlay with different blur values',
        (WidgetTester tester) async {
      final GlobalKey key1 = GlobalKey();
      final GlobalKey key2 = GlobalKey();
      final GlobalKey key3 = GlobalKey();
    ...
    testWidgets('Overlay with barrier click and disabled interaction',
        (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
      int barrierClickCount = 0;
        ...
        MaterialApp(
          home: Scaffold(
            body: Showcase(
              key: key,
              title: 'Barrier Test',
              description: 'Testing barrier with disabled interaction',
              onBarrierClick: () => barrierClickCount++,
    ...
    testWidgets('Overlay with complex target shape',
        (WidgetTester tester) async {
      final GlobalKey key = GlobalKey();
        ...
        MaterialApp(
          home: Scaffold(
            body: Showcase(
              key: key,
              title: 'Complex Shape',
              description: 'Testing overlay with complex target shape',
              targetShapeBorder: RoundedRectangleBorder(
                borderRadius: BorderRadius.circular(20),
    ...
    testWidgets('Overlay with zero opacity', (WidgetTester tester) async {
      ...
      final color = findOverlayColor(tester, Colors.black.withOpacity(0.0));
      expect(color, isNotNull);
      expect(color, Colors.black.withOpacity(0.0));
    });
    ...
    testWidgets('Overlay with full opacity', (WidgetTester tester) async {
      ...
      final color = findOverlayColor(tester, Colors.black.withOpacity(1.0));
      expect(color, isNotNull);
      expect(color, Colors.black.withOpacity(1.0));
    });
  ...
  group('Overlay Edge Cases', () {
    ...
    testWidgets('Overlay with fully opaque color', (WidgetTester tester) async {
      ...
      final color = findOverlayColor(tester, Colors.black.withOpacity(1.0));
      expect(color, isNotNull);
      expect(color, equals(Colors.black.withOpacity(1.0)));
